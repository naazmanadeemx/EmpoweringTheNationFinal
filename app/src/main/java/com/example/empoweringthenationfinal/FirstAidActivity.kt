...
class FirstAidActivity : AppCompatActivity() {
    ...
    private val courseFee = 1500 // Fee for the First Aid course
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Find views
        val selectFirstAidCheckbox: CheckBox = findViewById(R.id.select_first_aid_course)
        ...
        // Set a listener on the checkbox to add or remove the course from the selected courses
        selectFirstAidCheckbox.setOnCheckedChangeListener { _, isChecked ->
            if (isChecked) {
                // Add the course to the selected courses
                SelectedCoursesManager.addCourse("First Aid")
                Toast.makeText(this, "First Aid course added to cart", Toast.LENGTH_SHORT).show()
            } else {
                // Remove the course from the selected courses
                SelectedCoursesManager.removeCourse("First Aid")
                Toast.makeText(this, "First Aid course removed from cart", Toast.LENGTH_SHORT).show()
            }
        }
        ...
        calculateButton.setOnClickListener {
            ...
            if (selectedCourses.isNotEmpty()) {
                ...
                // Update the result TextView with the calculated total
                resultTextView.text = String.format("Total (with VAT): R%d", totalWithVAT)
            } else {
